...
module EachBatched
  ...
  # 
  #  * Note: this algorithm does NOT work well with data that may have inserts/deletes while you're looping,
  ...
  #  * Note: this algorithm may be slower than #batches_by_ids if your query doesn't execute very quickly.
  #  * Note: this algorithm can't be lazily loaded, because it checks for empty results to see when it's done.
  def batches_by_range(batch_size=DEFAULT_BATCH_SIZE)
    start_offset = scoped.offset_value || 0
    end_limit = scoped.limit_value # || nil
    group_number = 0
    processed_number = 0
    # This giant while condition (with multiple assignments in it) is annoying, isn't it!
    # But simplifying it means I have to repeat most of it multiple times!
    # And putting it into a subroutine doesn't really save space either, with too many parameters and/or return values!
  ...
  # 
  # Note: You should explicitly set an order if you want the same order as #batches_by_range, or it may be different.
  # Note: The yielded scope can be lazily loaded (though the id selection query has already run obviously)
